...
| Static public artifact | Build: Vite `static` mode + manual GitHub Pages workflow   | This repository; bundled snapshot                       | Produces `dist/static` without API calls or false fallback state. Pages is optional and not activated; any static host can replace it.                                                   |
...
server-side capability. Pages remains replaceable with any static host. The
workflow uses the official checkout, setup-node, configure-pages,
...
credential is required by this Build preparation, and no Pages or domain action
occurred.
